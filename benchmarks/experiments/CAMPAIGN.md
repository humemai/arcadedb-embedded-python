...
| 4 | A served bulk path for DOCUMENT and vector types (served document and vector arms load through `sqlscript` with the values in the text; `/batch` is vertex/edge JSONL). Measured 2026-09-25 on the laptop at `5f8b817097`, four paths, WAL on in all: plain rows 13k rows/s `sqlscript`, 21k bound `INSERT ... CONTENT :rows`, 12.6k Postgres wire `executeBatch`, 20.6k gRPC `BulkInsert`; with a 96-float vector 4.8k, 8.3k, 12.1k, 7.9k | ArcadeDB only | asked upstream as ArcadeData/arcadedb#8337 (which path, batch size, faster float arrays, WAL); follow their recommendation on every served arm at the re-pin | `.notes` repros/served-doc-ingest |
...
| 9 | The re-pin build: 26.10.1 with every fix we filed verified by its repro (#8232, #8260/#8265/#8266, #8285, #8288, PR #8269), comparators to latest stable | routine | steps 3 and 4 of the routine | `.notes` issues-trail |
