...
| 6 | Served SQL arms over the Postgres wire (psycopg) instead of HTTP (`requests`) | ArcadeDB only | #8288's fix is in ArcadeData/arcadedb#8308 (verified at its head, milestone 26.10.1); once it ships, re-measure on the bench host (the laptop cannot time below a millisecond), ask upstream, then the user's call | `.notes` repros/protocol-latency |
