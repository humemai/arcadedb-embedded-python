...
`qOA` -> `qOA2` -> `qOB` -> `qOC` -> `qOD` -> `qOI` -> `qOD2` -> `qOA3` -> `qOD3` -> `qOA4` -> `qOE` -> `qOF` -> `qOG` -> `qOH` -> `qOB2` -> `qOJ` -> `qOK` -> `qOL`.
...
| 6 | Served SQL arms over the Postgres wire (psycopg) instead of HTTP (`requests`) | ArcadeDB only | blocked: needs ArcadeData/arcadedb#8288 fixed; re-measure on the bench host (the laptop cannot time below a millisecond), ask upstream, then the user's call | `.notes` repros/protocol-latency |
| 7 | What the memory column means: the peak, the working set after a full GC, or the peak with the heap printed beside it | every JVM arm | the user decides | HANDOFF, "Owed at the next re-pin: decide what the memory column should MEAN" |
...
| 9 | The dense table's ingest and index phases partition the total | every dense arm's timers | owed, disclosed on the page until then | BUGS F101 |
| 10 | A cold-pass estimate per lane and tier against its cap, before a stage runs | tooling, no row changes | owed | BUGS F74 |
| 11 | A publish gate refusing more than one version per comparator per page | tooling | owed | BUGS F61 |
| 12 | The re-pin build: 26.10.1 with every fix we filed verified by its repro (#8232, #8260/#8265/#8266, #8285, #8288, PR #8269), comparators to latest stable | routine | steps 3 and 4 of the routine | `.notes` issues-trail |

**Retired: settled with no change, listed so nobody re-opens them.** ArcadeDB's single-bucket default (four buckets make its analytics 24-35% slower today; revisit only if ArcadeData/arcadedb#8265's second step lands). Raising ArcadeDB's statement-cache settings (a knob only ArcadeDB would get; item 2 removes the literal path instead). The sparse ingest path (already the faster one). Durable commit cost (one sync per commit, equal to SQLite's; not a defect). `expected_edge_count` as a speed lever (no measurable effect; item 1 adopts it only because it is the documented setting).
