...
**The October chain on mini, at pin `417314c18`.** Sixteen stages generated by `make_october_stages.py` from one table, each waiting on its predecessor's `<id> ALL-DONE` marker in `~/STATUS.txt`:

`qOA` -> `qOA2` -> `qOB` -> `qOC` -> `qOD` -> `qOI` -> `qOD2` -> `qOA3` -> `qOD3` -> `qOA4` -> `qOE` -> `qOF` -> `qOG` -> `qOH` -> `qOB2` -> `qOJ`.
