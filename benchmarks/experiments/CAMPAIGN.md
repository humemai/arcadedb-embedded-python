...
`qOA` -> `qOA2` -> `qOB` -> `qOC` -> `qOD` -> `qOI` -> `qOD2` -> `qOA3` -> `qOD3` -> `qOA4` -> `qOE` -> `qOF` -> `qOG` -> `qOH` -> `qOB2` -> `qOJ` -> `qOK`.
...
**What the re-pin owes the HARNESS, and not just the engine (2026-09-23).** Three changes are landed in code and deliberately not in the campaign, because the lanes they touch have already been measured and a mid-run change splits the rows. Every one of them moves a PUBLISHED number, so the re-pin is where they take effect and the re-pin checklist is here rather than in a commit message nobody re-reads:

* **The graph lane must ask with PARAMETERS, not interpolated ids.** `graph_common.py` builds every read from one template with `{id}` in the text, so no Cypher engine on that table ever reuses a parsed plan and the point-lookup column is substantially a comparison of PARSERS. Measured at the pin: Cypher with an embedded literal 0.8702 ms against 0.0907 ms with `$id`, and SQL 0.4810 against 0.0910 -- with parameters the two languages are indistinguishable (1.00x) and both are 5-10x faster. Upstream told us this before we found it (ArcadeData/arcadedb#8132), and it applies to every engine on the lane, not only ours. NOT yet changed: it is the one of the three that rewrites query construction for eleven arms, and it wants its own smoke.
* **The cross-model filtered search pre-filters** (BUGS F112, done in code). The arm post-filtered because a probe tried five BARE function names and concluded a namespaced capability was absent; published `filtered recall@10` of 0.013 against every other engine's 1.000 was our query shape, not the engine.
* **The cross-model bulk load keeps its write-ahead log on** (BUGS F113, done in code). `graph_batch` relaxes durability for a bulk load by default; no comparator's loader does, and keeping it on measured 0.97x -- free.

Verified together on the re-pin's own engine after syncing upstream: `recall_filtered` 1.0, no durability relaxation, and the ingest/index timers still partition their total.
