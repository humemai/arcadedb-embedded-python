...
| 10 | ArcadeDB's bucket count. The fairness rule runs every engine at its defaults, and ArcadeDB's type default is one bucket, so its SQL scans never run in parallel. Until ArcadeData/arcadedb#8265 that was harmless (four buckets made analytics 24-35% slower); with #8265 merged (26.10.1), four buckets make a full-scan `GROUP BY` about 1.4x faster than one on 4 cores (a filtered `count(*)` does not gain) | ArcadeDB only | ask upstream whether the default should change (the benchmark follows the default either way); tuning ArcadeDB alone is the user's call | `.notes` repros/groupby-scan |
