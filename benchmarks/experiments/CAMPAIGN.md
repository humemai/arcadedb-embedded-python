...
**The October chain on mini, at pin `417314c18`.** Ten stages generated by `make_october_stages.py` from one table, each waiting on its predecessor's `<id> ALL-DONE` marker in `~/STATUS.txt`:

`qOA` -> `qOAR` -> `qOA2` -> `qOB` -> `qOC` -> `qOD` -> `qOD2` -> `qOA3` -> `qOD3` -> `qOI` -> `qOE` -> `qOF` -> `qOG` -> `qOH` -> `qOB2`.
