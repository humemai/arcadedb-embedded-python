...
* **l3d, before `qOH`** (which had not run, so no October dense row splits): F134, SurrealDB's served index defined after the load. The 3.2.4 server builds an index defined on the empty table in the background, so the build timer stopped before the index existed and queries were timed inside the build (September published this arm at 1,510 ms p50 at 1M). Landed on main (`90eb1ffbe1`).
* **e2, via `qOM`** (hand-written on mini, waits on `qOL`, refuses to start unless the three fixes are in the tree and `dbbench:composed` carries Qdrant 1.19.1): F132, SurrealDB's candidate ranking scanned every product instead of addressing record ids (442 ms published, 15.7 ms fixed on the laptop); F133, the composed stack's vector half was the Qdrant client's pure-Python local mode, now the pinned Qdrant server in one container with Neo4j (`Dockerfile.composed`); F134 for the served e2 arm. Re-runs SurrealDB embedded at e2, SurrealDB served at both sizes, the composed stack at both sizes with its atomicity trial, REPS=5 (`d3f790fa84`). Until it lands, `/next` withholds SurrealDB's graph-filtered search and its served e2_500k retrieval and withdraws the composed stack's e2 row (its atomicity row stays).
...
| 3 | Dense ingest through `graph_batch(use_wal=True)` with Java float arrays | ArcadeDB only | **decided 2026-09-26 (DECISIONS #116): adopt at the re-pin** (1.4x) | HANDOFF vector-lane audit |
