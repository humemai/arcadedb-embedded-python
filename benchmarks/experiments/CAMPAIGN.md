...
* **e2, via `qOL`** (hand-written, waits on `qOK`, refuses to start unless all three are in the tree it pulls): F112 the post-filter (published `filtered recall@10` 0.013 against everyone else's 1.000), F113 the write-ahead log off during the bulk load, and F116 the build handing JPype Python lists instead of Java arrays (4.29x on the vertex load).

**What the re-pin owes the HARNESS, and not just the engine (2026-09-23).** These are measured or found but deliberately NOT in this campaign, because the lanes they touch have already been measured and no stage re-runs them; a mid-run change would split the rows. Each moves a PUBLISHED number, so the re-pin is where they take effect, and the list is here rather than in a commit message nobody re-reads:
