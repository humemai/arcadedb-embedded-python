...
Extending a published campaign (DECISIONS #103a, #103b). After its chain ends, a campaign may still gain new comparators, and a table's size may be raised, at its own pin and instrument. A new comparator is new rows. A raised size replaces the table's old tier: every engine on that table is re-run at the new size, the old tier's rows leave the page and stay in the frozen archive, and no table gains a second size or a second table to hold the old one (the user, 2026-09-17: "if we change the datasize for X then the old datasize will be gone"). What is never done is to change the corpus under a cell that keeps its label, because then the page could no longer say what it compares. How many sizes a table carries (DECISIONS #103c): one by default. A table carries a second size only where scaling is the thing being shown, a latency flat from the small tier to the large one meaning the engine indexes and one that climbs meaning it scans; that is why graph, dense, and sparse carry two and documents transactional, time series, and cross-model carry one. Each size is its own set of rows with its own label read from the rows; a second size is never a second table. A new comparator's arm is added as a new backend without touching any existing arm, linted and smoked on the laptop, and only then queued.
