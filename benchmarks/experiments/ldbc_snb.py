...
# (DECISIONS #104): SF1's full 9,892-person KNOWS graph makes q3/q6/q9 (the
...
# and both engines it compares see the identical capped graph.
...
# nine queries join the five hand-written ones). Loaded ONLY for the olap
# workload, keyed off BENCH_GRAPH_SOURCE=ldbc the same way the persons+KNOWS
# projection is; the interactive (oltp) workload keeps loading persons+KNOWS
# alone and never sees any of this.
#
...
# message half is layered on top of them.
#
...
# are never dangling and need no filtering.
import os as _os_msg

MSG_LIMIT = int(_os_msg.environ.get("BENCH_GRAPH_MSG_LIMIT") or 0)
