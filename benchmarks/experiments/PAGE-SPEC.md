...
- **Ingest is a column, not a table (2026-09-11).** The user asked for it as columns on the tables that own the rows: `ingest records/s` and `ingest s` on documents, `ingest pts/s` on time series, `build s` on both vector tables, and `load s` on the graph, TPC and cross-model tables (from `build_s`). A separate side-by-side table was built and dropped the same day.
