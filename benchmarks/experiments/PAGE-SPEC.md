...
| id | title | rows | columns |
|---|---|---|---|
| `l2` | Graph OLTP | ArcadeDB embedded and server, ArangoDB, DuckPGQ, FalkorDB, LadybugDB, Memgraph, Neo4j, SurrealDB embedded and server (all published; DuckPGQ, FalkorDB and Memgraph joined at SF1 and SF10 in qEB) | point, 1-hop, 2-hop, 3-hop with a property filter, insert, update, and delete, each p50 and p99, ingest vertices+edges/s, ingest total s, peak memory GiB, disk GiB |
| `l2olap` | Graph analytics with and without the view | ArcadeDB embedded, embedded GAV, server, server GAV, ArangoDB, LadybugDB, Neo4j, SurrealDB embedded and server (all published; the SurrealDB embedded SF10 cell exceeded its budget and is named on the table) | the five queries (average friend age, friendships within a city, most friends, degree distribution, and triangle count), each cold and warm p50 and p99, view build s, ingest vertices+edges/s, ingest total s, peak memory GiB, disk GiB |

Scales: `l2` SF1 + SF10 (the interactive table keeps the persons-and-KNOWS projection at both sizes: its per-seed reads never touch the message half, and the full network at SF10 is not feasible); `l2olap` SF1 + SF10 on the September page. October moves the analytics table to the full SF1 network, `sf1full`, as its single size: the full network at SF10 is not feasible, so this is the one table #108 leaves at one size. September's attempt did not switch -- qEB measured nine engines at `sf1full` and SurrealDB embedded was killed at the tier's memory envelope -- so the live `only_scales` still reads `{"sf1", "sf10"}` and those nine rows publish in October, where an engine absent from a size is a declared outcome rather than a block (#108).
...
| id | title | rows | columns |
|---|---|---|---|
| `docs_oltp` | Document OLTP | ArcadeDB embedded and server, ArangoDB, DuckDB, MongoDB, PostgreSQL, SQLite, SurrealDB embedded and server (all published) | new-order and payment, and one order line inserted, read by key, updated, and deleted, each p50 and p99, OLTP ops/s over new-order and payment, ingest documents/s, ingest total s, peak memory GiB, disk GiB |
...
| id | title | rows | columns |
|---|---|---|---|
| `e2` | Cross-model transaction | ArcadeDB embedded and server, Qdrant + Neo4j, SurrealDB embedded and server, PostgreSQL + pgvector + AGE, Neo4j vector index , ArangoDB (all published; qDT moved the composed Qdrant + Neo4j row to Neo4j 2026.07.1, but its Qdrant half still runs in memory and October is where that is retired, PROTOCOL.md section 7) | the composed write, the same write interrupted, the retrieval path, and the graph-filtered vector search, each p50 and p99, recall on the two read paths, ingest+index vertices+edges/s, ingest+index total s, peak memory GiB, disk GiB |
