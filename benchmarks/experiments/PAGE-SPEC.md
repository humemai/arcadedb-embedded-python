...
- **Ingest and build in one table (`ingest`, 2026-09-11).** Every load the harness times, side by side: streamed document inserts, TPC-H load, graph vertices and edges, time-series points, dense and sparse index builds, the cross-model set. `records/s` where the lane records a rate (graph derived from persons plus edges over load seconds), `load s` everywhere, peak memory and disk. Dense rows take their build from the dense table's overlay so the page has one number per build. The row label carries the group ("Documents, streamed inserts: DuckDB") because the renderer has no workload column; scales are `lane:scale` so the final ordering pass keeps the groups together.
