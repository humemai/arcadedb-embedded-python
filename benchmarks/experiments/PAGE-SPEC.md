...
- **Ingest is a column pair, not a table.** Every table whose rows loaded something shows `ingest <unit>/s` and `ingest total s`; on the vector and cross-model tables the timer includes the index build and the note says so. The dense and sparse lanes already record `ingest_s` and `index_s` separately where the engine has the boundary, additive and with `build_s` unchanged (DECISIONS #74 item 2); the page keeps the single ingest+index pair until October splits the columns. Derived rates use `_rate(count_fields, seconds_field)` in the spec, aggregated per row like any field.
