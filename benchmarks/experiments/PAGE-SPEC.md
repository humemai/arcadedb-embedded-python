...
The columns below are the 2026-10 instrument's, which carries the forty-nine operations of PROTOCOL.md section 2 (documents 6 OLTP and 5 OLAP, graph 7 OLTP and 14 OLAP, dense 3, sparse 1, time series 5 beside the one-hour range the lane already ran, cross-model 4, and lifecycle 4) and the one measurement set of DECISIONS #89 as amended: one cold column per CELL (`cold_first_query_ms`) and warm latency per query at p50 and p99, throughput where the operation has a rate, recall where the index is approximate, peak memory, on-disk size after the workload, and ingest and index build as separate columns on the vector tables. A table that omits one of those states why in its conditions, which `page_check` enforces. An operation an engine cannot express is a declared absence (DECISIONS #88): the cell is a dash and the condition names the engine and the operation, so it cannot be read as a slow engine or as a gap still to be filled. **A dash means that and only that (DECISIONS #111).** An engine that RAN and did not finish keeps its row and prints the outcome in the cell -- `>2h` for the tier's cap (derived from `runner.TIMEOUT_BY_SCALE`, so `>15m` through `>48h`), `OOM` for an envelope failure, `lost` for a dropped connection, `err` for a failure inside budget whose cause will not classify, `wd` for a withheld wrong answer -- because a vanished row reads as an untested engine, which flatters whichever engines finished. The legend is generated from the marks present and refuses one it cannot define. A harness failure never gets an engine mark, and a cell skipped because we expected it to be slow is published as its bound or not at all, never as a blank. The queue chain that landed the arms named below is CAMPAIGN.md section 6, which also records the September extension's two stages and the three that were cancelled. Tables planned and not built: `l3d_params` (matched operating points; waits on a renderer for text cells), `l4_tentag` (one-tag against ten-tag, ratios only), `pyingest` (the write side of the Python cost), `pysweep` (where the Python tax comes from), `ops_recovery`, `ops_failover`, and `ops_start` (measured, never published). Retired and not coming back: `l3smp` (folded into `l3s` as warm columns), `l1`, `l1olap`, `l1tpc` (the synthetic document set is no longer run, DECISIONS #67 and #72), `l3s_nocompact` (the settle-step ablation is operator material, not page material, DECISIONS #71), `ops_build` and `ops_disk` (realised as the ingest pair and the disk column on every table).

**Sizes (DECISIONS #108, amending #103b and #103c).** Each table carries TWO sizes wherever a second one is affordable and informative, because one point cannot tell a query that moved elevenfold from one that did not move at all. #103c's "one size by default" and #103b's replacement rule are both retired: the large size is a ROW GROUP beside the small one rather than a switch, and an engine with no row at it is a declared outcome under #103g (censored, envelope failure, or failed inside budget) rather than a reason to withhold every other engine's rows. Graph analytics is the single one-size exception, the full SF1 network, because the full network at SF10 is not feasible. No table accumulates two generations of ONE size and no new table is made for a size. The skeleton runs every lane at its smallest laptop corpus with the size column naming that corpus (`export_web.SKELETON_SCALE_LABELS`), so the sizes named below are the campaign's, and the skeleton's labels say "skeleton" so the two cannot be confused.
...
| `l3d` | Dense vector search | ArcadeDB embedded and server, each int8 and fp32; ArangoDB (fp32), Chroma (fp32), DuckDB VSS (fp32, DuckDB 1.5.4), LanceDB (int8), Milvus (int8 and fp32), MongoDB (fp32, through MongoDB Search in the same container), Neo4j (fp32), pgvector (fp32), Qdrant (int8 and fp32), sqlite-vec (int8 and fp32), SurrealDB embedded and server (fp32). The SurrealDB embedded 1M cell exceeded its budget and is named on the table, with qDX's phase-marked re-run recording the phase it dies in (BUGS.md F41); ArangoDB's DEEP-10M cells are withheld below the recall floor (BUGS.md F55) | cold p50 ms, cold p99 ms, after insert p50 ms, after delete p50 ms, insert into index ms/vector, recall@10 after insert, delete from index ms/vector, recall@10 after delete, index s, recall@10, ingest+index vectors/s, ingest s, ingest+index total s, peak memory GiB, disk GiB. Warm columns come from the dense multipass driver on the bench host and are absent from the skeleton |
...
Sizes: `l2` two, SF1 and SF10 of the persons-and-KNOWS projection, since per-seed reads never touch the message half and the full network at SF10 is not feasible. `l2olap` is the ONE table #108 leaves at a single size, the full SF1 network (`sf1full`), which LSQB's nine need and which SF10 cannot carry. The September page still prints the SF1 and SF10 projections there: qEB measured nine engines at `sf1full` and SurrealDB embedded was killed at the tier's memory envelope, so under #103b the switch was blocked and `export_web` `l2olap` `only_scales` still reads `{"sf1", "sf10"}`. Under #108 an engine absent from a size is a declared outcome rather than a block, and those rows publish in October at October's pin. The skeleton's analytics rows read a capped LDBC SF1 slice and are labelled as such.

### Documents and time series

The page's document tables are TPC only (DECISIONS #67): `docs_oltp` and `docs_olap`, with PostgreSQL beside the default arm. September prints TPC-H SF1 on both. October carries TPC-H SF1 AND SF10 on both (#108), the transactional table's SF10 being the first to drop if the matrix does not fit. September's attempt to move the analytics table to SF10 alone did not switch: qEA measured DuckDB and both PostgreSQL arms there, three engines were censored at the tier's cap, SurrealDB served lost its connection and SurrealDB embedded was killed at the tier's memory envelope. Under #103b that blocked the raise and the SF10 outcomes appear as notes under SF1 rows; under #108 those three measured arms publish as an SF10 row group with the other engines' absences declared by outcome.
...
The ingest column on `docs_oltp`, `docs_olap`, `l4` and `e2` becomes THREE columns when the re-runs carrying the 2026-09-22 index work land (FAIRNESS F14): `ingest s` and `index s` beside `ingest total s`, which keeps its name and its meaning, the whole of `build()`. The inventory above lists these tables as the skeleton payload shows them today, with one ingest column, because a row that records no `index_s` does not fill the split and a column no row fills is dropped. Those tables then carry a generated sentence saying that the total is the two parts added, because `ingest s` and `ingest total s` sitting side by side is otherwise a reader's guess. The sentence rather than a longer column name: the label is a key in several places that are typed rather than derived, and renaming it there dropped the column from both document tables instead of renaming it.
...
Scales (#108): `docs_oltp` and `docs_olap` TPC-H SF1 + SF10, `l4` 100 + 1,000 hosts. September prints one size on each of the three; October carries both, and `docs_oltp`'s SF10 is the first size to drop if the matrix does not fit. The time-series lane parses the line protocol in Python inside every cell, which is about 29 minutes of the 1,000-host tier before any engine work, so that size is priced as parse plus engine and not as engine alone. **And 1,000 hosts is near the envelope, not comfortably inside it:** measured 2026-09-20, the ArcadeDB document path held 24.4 GiB of its 32 GiB cap at that size. Any proposal for a larger time-series tier starts from that number, because an engine at three quarters of its allowance at one size is an ENVELOPE failure at the next -- which is exactly how SurrealDB embedded died twice in September's extension (#103g), and an envelope failure is evidence for nothing about latency in either direction.
...
Scales (#108): `e2` 50k + 500k products. September prints 50k alone; the 500k raise was queued as qED and cancelled with the rest of the extension, so it is October's, as a second row group rather than a replacement.
...
- **No sentence on the page cites a document the reader cannot open.** Our prose carries its own paper trail -- `(DECISIONS #82b, #100)`, `(BUGS F55)`, `FAIRNESS F1` -- and those citations were going out on the page: forty-seven of them on the preview's graph analytics table alone, found by the user reading `/next` on 2026-09-21. `export_web` strips them from every reader-facing string at the single point the payload is written, rather than at the forty places the sentences are built, because those live in three modules and the next sentence written would have leaked again. What the stripper cannot repair it REFUSES to publish (`_refuse_internal_refs`), because a sanitiser that silently misses a form is worse than none: nothing would look wrong. Fields that name an artifact on purpose are provenance and stay exact -- the generator's own path, a table's source files. The rule is about the reader, not about secrecy: the reasoning belongs on the page, the filing number does not.
...
- **A cell with no row is accounted for by outcome, and the outcomes are not the same thing.** `export_web._censored_notes` reads every failed cell out of `runs.jsonl` and writes one sentence per cell naming the engine and the size. Three kinds, told apart on the row and worded differently: a cell that ran past the tier's cap is CENSORED and the note names the cap, which is a true statement about that engine at that size; a cell that failed inside its budget for another reason carries what it reported, so a lost connection does not read as slowness; and a cell the kernel killed at the tier's memory envelope (`rc=137` with `oom_killed`) is an ENVELOPE failure, which says the engine did not fit the cap every engine on that tier was given, and never that it was slow. An envelope failure is not evidence for a latency claim in either direction, and the page must not let it read as one.
- **An outcome note may name a size the table does not print.** A raised tier replaces a table's rows only when every engine on that table has landed there (#103b); until then the table keeps the size it has while the notes name what the attempt at the larger size did. Both raises attempted in September's extension stopped that way, so `docs_olap` prints TPC-H SF1 under notes about SF10 and `l2olap` prints the SF1 and SF10 projections under a note about the full network. The note said "there is no row", which is what a reader needs AFTER they read the note and not while they read the table; under #111 the engine keeps its row and the cell carries the outcome, and the note explains it; what it does not say is that some engines DID measure there, so a table in that state is a half-finished raise and is read as one. This describes the SEPTEMBER page and stays true of it. October does not reach that state: under #108 a table carries both sizes, the large size is a row group rather than a replacement, and an engine absent from it is a declared outcome under #103g instead of a reason to withhold every other engine's rows.
...
**Six gates run.** `refresh_web_page.GATES` is `equivalence_check`, `provenance_check`, `fairness_check`, `page_check`, `version_consistency_check`, `version_pin_check`. The sixth (2026-09-21) is the only one that reads no rows: the other five ask whether the measurements are sound, and it asks whether the four files a human edits can still disagree about which artifact was measured -- 86 pins across `runner.py`, `build_images.sh` and the two Dockerfiles, with COMPARATORS.md stating them for a reader, and nothing compared them until `dbbench:pg-age` spent a campaign on PostgreSQL 17.11 under an instrument declaring 18.6 (BUGS F76). It found two real disagreements on its first run. Its `--runtime` mode is not one of the six and reads no rows either: it runs the built images and reads their versions back out, and it runs as a STAGE PREFLIGHT rather than at freeze, because the pg_age defect cost two hours of machine time per cell and a check that can only run after the run is an autopsy (BUGS F90). The fifth refuses a page where one engine wears two version strings or a comparator wears an ArcadeDB release number, which the dense table published for months before anything looked (BUGS F63). Until the branch integration landed, `main` ran the last three: `equivalence_check` lived only on `october-instrument`, along with the answer digests its lanes stamp and the `*_digest` fields a frozen row needs, and no row in `runs_paper.csv` carries one. So the SEPTEMBER page was published without any cross-engine answer comparison, which is a stated design fact and not an oversight -- nothing asked that question before 2026-09-14, and the September rows are built on adapters that were never checked for a dropped filter, group, or join condition. The answer gate therefore has nothing to compare on a September freeze and everything to compare on October's, which is a statement about the ROWS and no longer about the branch.
...
| `l3d` | Dense vector search | ArangoDB (fp32), ArcadeDB (embedded, fp32), ArcadeDB (embedded, int8), ArcadeDB (server, fp32), ArcadeDB (server, int8), Chroma (fp32), DuckDB VSS (fp32), LanceDB (int8), Milvus (fp32), Milvus (int8), Neo4j (fp32), Qdrant (fp32), Qdrant (int8), SurrealDB (embedded), SurrealDB (server), SurrealDB (server, fp32), pgvector (fp32), sqlite-vec (fp32), sqlite-vec (int8) | 1M vectors, 9.99M vectors | cold p50 ms, cold p99 ms, warm p50 ms, warm p99 ms, recall@10, ingest+index total s, ingest+index vectors/s, peak memory GiB, disk GiB |
| `l2` | Graph OLTP | ArangoDB, ArcadeDB (embedded), ArcadeDB (server), DuckPGQ, FalkorDB, LadybugDB, Memgraph, Neo4j, SurrealDB (embedded), SurrealDB (server) | SF1 (10k people, 181k friendships), SF10 (66k people, 1.9M friendships) | point p50 ms, point p99 ms, 1-hop p50 ms, 1-hop p99 ms, 2-hop p50 ms, 2-hop p99 ms, write p50 ms, write p99 ms, ingest vertices+edges/s, ingest total s, peak memory GiB, disk GiB |
| `l2olap` | Graph OLAP | ArangoDB, ArcadeDB (embedded), ArcadeDB (embedded, GAV), ArcadeDB (server), ArcadeDB (server, GAV), LadybugDB, Neo4j, SurrealDB (embedded), SurrealDB (server) | SF1 (10k people, 181k friendships), SF10 (66k people, 1.9M friendships) | ingest vertices+edges/s, ingest total s, average friend age p50 ms, average friend age p99 ms, friends in same city p50 ms, friends in same city p99 ms, most friends p50 ms, most friends p99 ms, peak memory GiB, disk GiB |
| `e2atom` | Cross-model transaction: what survives a crash | ArangoDB, ArcadeDB (one transaction), ArcadeDB (server, one transaction), Neo4j (vector index), PostgreSQL + pgvector + AGE, Qdrant + Neo4j (no shared transaction), SurrealDB (embedded), SurrealDB (server) | 50k products | trials, torn results |
| `e2` | Cross-model transaction | ArangoDB, ArcadeDB (one transaction), ArcadeDB (server, one transaction), Neo4j (vector index), PostgreSQL + pgvector + AGE, Qdrant + Neo4j (no shared transaction), SurrealDB (embedded), SurrealDB (server) | 50k products | p50 ms, p99 ms, ingest+index vertices+edges/s, ingest+index total s, peak memory GiB, disk GiB |
...
| `docs_oltp` | Document OLTP | ArangoDB, ArcadeDB (embedded), ArcadeDB (server), DuckDB, MongoDB, PostgreSQL, SQLite, SurrealDB (embedded), SurrealDB (server) | TPC-H SF1 (6.0M line items) | new-order p50 ms, new-order p99 ms, OLTP ops/s, ingest documents/s, ingest total s, peak memory GiB, disk GiB |
| `docs_olap` | Document OLAP | ArangoDB, DuckDB, MongoDB, PostgreSQL, SQLite, SurrealDB (embedded), SurrealDB (server) | TPC-H SF1 (6.0M line items) | Q1 p50 ms, Q1 p99 ms, Q6 p50 ms, Q6 p99 ms, ingest documents/s, ingest total s, peak memory GiB, disk GiB |
