...
Scales: `l2` SF1 + SF10 (the interactive table keeps the persons-and-KNOWS projection at both sizes: its per-seed reads never touch the message half, and the full network at SF10 is not feasible); `l2olap` SF1 + SF10 (SF10 alone cannot show whether the view's benefit scales). The analytics table is due to move to the full SF1 network, `sf1full`, and did not: qEB measured nine engines there and SurrealDB embedded was killed at the tier's memory envelope, so `export_web` `l2olap` `only_scales` still reads `{"sf1", "sf10"}` and the switch is October's (#103b).
...
The page's document tables are TPC only (DECISIONS #67): `docs_oltp` and `docs_olap`, with PostgreSQL beside the default arm. Both print TPC-H SF1. The analytics table is due to move to SF10 alone and did not: qEA measured DuckDB and both PostgreSQL arms there, three engines were censored at the tier's cap, SurrealDB served lost its connection and SurrealDB embedded was killed at the tier's memory envelope, so the raise did not meet #103b's bar and the SF10 outcomes appear as notes under SF1 rows.
