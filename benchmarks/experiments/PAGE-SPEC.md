...
Scales (#108): `docs_oltp` and `docs_olap` TPC-H SF1 + SF10, `l4` 100 + 1,000 hosts. September prints one size on each of the three; October carries both, and `docs_oltp`'s SF10 is the first size to drop if the matrix does not fit. The time-series lane parses the line protocol in Python inside every cell, which is about 29 minutes of the 1,000-host tier before any engine work, so that size is priced as parse plus engine and not as engine alone.
