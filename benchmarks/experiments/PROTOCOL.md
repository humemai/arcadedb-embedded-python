...
| Milvus dense (L3d) | flush, wait for the HNSW index (state Finished AND pending 0 AND indexed == total), then forced `compact()` until the persistent segment count stops shrinking with the index re-waited after each pass (the image sets `indexBasedCompaction=true`, so unindexed segments are never merged), then load; all inside the timed build | settle step | Without the forced compaction the rows split 3/2 into two segment layouts under one "Finished" flag (BUGS F8): the interim index served the merged segment at 3.7 ms / recall 0.994 or the real HNSW at 1.6 ms / 0.986. The forced compaction is the steady state Milvus reaches on its own; we stop racing it. Adds to the build time. | DISCLOSED here and in the T5/page condition; `milvus_persistent_segments`, `milvus_compactions_forced` on every row |
