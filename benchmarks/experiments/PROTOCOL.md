...
- A served row names a `d.d.d` version or carries an `@sha256` digest. `server:latest`, `server` and `unknown (PackageNotFoundError)` are not versions and the pre-release guard cannot see through them. **GATE** `load_canonical` for `runs.jsonl` rows; **GATE** `_engine_is_identifiable` in `export_web`: an ArcadeDB row whose engine string and commit identify no build is withheld
...
- Lane scripts publish; bespoke drivers investigate. Both known protocol violations were bespoke-driver rows promoted to cells, and T5's dense half is still an overlay from `dense_multipass_driver.py` (run through `runner.py --driver`); the native time-series row moved into `l4_tsbs.py` in September 2026. **GATE** `fairness_check.report_producers` (F6b) for `runs.jsonl` lanes, which sees no overlay record and knows neither `l3d_mp` nor `l4n`; **REMEMBERED** for every overlay-fed cell
...
- Every artifact a paper cites lives in this repository, and the superseded run is not left where it can be compared against. **GATE (report only)** `provenance_check.FEEDS`: an empty feed prints "(no result files)" and counts nothing. **GATE**: every pinned reader refuses when `<name>_<pin>` is missing; legacy overlays archived 2026-09-07
...
- Ingest timers must stop on the same completion semantics across a lane, or the rates price different work. Resolved 2026-08-22 (`QuestTS.settle()` runs outside the timer); rows re-measured at the pin. **REMEMBERED**: a new time-series adapter must stop its timer at the same semantics
...
- Normalize a served topology before comparing: total is `srv_cap / mem_split`, never client + server. Addition read 1.75x the envelope and failed five compliant tiers. **GATE** `_total_envelope` in `fairness_check` (arithmetic splits and the full+client case)
...
| ArcadeDB embedded (L1-TPC, E2, L4 doc) | `heap_size` sets `-Xmx` only, `-Xms` left at the JVM default | resource fitting (absent) | The paper asserts `-Xms=-Xmx` as blanket protocol. False for three published arms whose server counterparts are pinned. | resolved; every embedded arm pins -Xms=-Xmx (`l1_tpc.py`, `e2_hybrid.py`, `l4_tsbs.py`), so the paper's protocol paragraph is now true |
| QuestDB (L4) | WAL-apply poll runs as a settle step after the timer | settle step | Resolved 2026-08-22 (`QuestTS.settle()` runs outside the timer); the l4 page condition says no engine settles inside the timer. | l4 page condition |
...
| ArcadeDB native TS | `TS_PRIMITIVE=1` (lane default 1); `TS_NUMPY` already defaults to 1 | escape hatch | One opt-in fast path under the 1.86M pts/s headline; comparators use ordinary bulk paths. `TS_NUMPY=0` exists only to reproduce the pre-fix number. | row field `ts_path`, page row label 'native time series' |
...
| ArcadeDB (L1-TPC) | ArcadeDB, SQLite, MongoDB, and SurrealDB index `l_shipdate`; DuckDB and PostgreSQL get none | operating-point | Both published TPC-H queries filter that column. Bias runs for us; PostgreSQL would benefit most. | **NOWHERE** |
...
| SurrealDB, composed Qdrant (E2) | `mem://` and `location=":memory:"` while ArcadeDB runs on disk | **unsanctioned**, being retired | The published rows still are in memory. SurrealDB's re-run on the SDK's SurrealKV disk store and the 3.2.4 server on RocksDB is queued (qDN, 2026-09-11); the composed stack's Qdrant half stays in memory until its own re-run. Disclosed on the page's cross-model table. | page table note |
...
| PostgreSQL (`postgres_tuned`) | shared_buffers etc derived from the container cap, durability untouched | resource fitting | Ablation arm asking whether image defaults make the latency comparison unfair. Runs on the document tables since the September campaign as "PostgreSQL (tuned)" beside the default arm. | page rows |
| SQLite (documents, TPC, time series) | `PRAGMA foreign_keys=ON; journal_mode=WAL; synchronous=NORMAL` | operating-point, user decision (#70) | The defaults fsync twice per commit; WAL+NORMAL is the common production setting. Makes SQLite faster on the write rows, so the knob runs against us. No SQLite row was measured at the defaults. | page durability note, COMPARATORS.md |
...
| Milvus dense (L3d) | `dataCoord.segment.sealProportion=0.5` (image default 0.12) via a mounted `milvus.yaml` that differs from the image's in that one line; then flush, wait for the HNSW index (Finished AND pending 0 AND indexed == total), a compaction pass that is a no-op by then, load; all inside the timed build | resource fitting + settle step, **disclosed** | The image seals growing segments at 12% of 1024 MB, so a 10M ingest lands as 26-28 segments of ~150 MB that Milvus's own compaction merges to 6-8 at an unpredictable moment; a manual `compact()` afterwards merges nothing (BUGS F8, three attempts). Sealing at 50% produces the merged layout directly, so every run starts in the steady state. Applied to fp32 and INT8; the sparse lane is not bimodal and stays at the image default. | DISCLOSED here and in the page condition; `milvus_segments_trace` on every row |
...
| ArcadeDB dense fp32 arms, T5 / page 10M multipass table only | `graphBuildCacheSize=9,990,000` (whole corpus), explicit; **INT8 arms at the engine default 100,000** | operating-point, **flatters us**, user decision (DECISIONS #56) | The multipass re-run at 8d6af9475 pins the HNSW build cache to the corpus on the two fp32 arms so the served fp32 build takes ~56 min instead of ~190 at the default. INT8 is left at its default because the user judged it immaterial there (a pinned 3.67M bought 12%). Comparators have no such knob. The single-pass campaign rows stay at the default. | MUST be in the T5 caption and the page condition; #7146 documents the default's cost |
