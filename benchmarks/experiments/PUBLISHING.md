...
Pruning comes before the documentation pass, not after, and the documentation pass is not optional: removing a rule leaves sentences elsewhere that referred to it, and a document describing something that no longer exists is worse than no document. This project has twice confused itself by adding entries and never removing them, which is why the check reports what should go as well as what should change. The check is a report rather than a gate on purpose. Most of what it lists is a judgement call, since a file naming the old pin may be recording history rather than describing the present, and a tool that decided that for you would be wrong in both directions.
