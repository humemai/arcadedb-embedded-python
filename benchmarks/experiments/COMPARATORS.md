...
| DuckDB | `dbbench:duckdb` (duckdb==1.5.4; 1.5.5 until 2026-09-17, re-pinned one patch DOWN so the DuckPGQ graph arm can run, DECISIONS #103d and #103e; every September DuckDB row is re-measured at 1.5.4 before the 1.5.5 rows leave the page) | 1.5.4 | documents, time series, dense (VSS), graph (DuckPGQ) | embedded | DataFrame / Arrow INSERT SELECT |
