...
## In the harness (2026-09-13; the pins every queued script runs at)
...
| SurrealDB (served) | `surrealdb/surrealdb@sha256:6a5002363ff5b000b72a55f985203e951e3175e578002954b0e38f113e48a698` (`v3.2.4`) | 3.2.4, RocksDB storage | documents (TPC-C/TPC-H), graph (RELATE edges), dense (HNSW), cross-model (re-run, replacing the in-memory row) | served | SDK `insert()` and bulk relation insert over ws. Disk-backed and served like the other comparators; `mem://` stays only as history. No time-series type: not measured there. The embedded twin (SDK, core 2.3.10 (SDK 2.0.0) on SurrealKV) runs the same three single-model lanes; the two versions differ because no newer SDK exists, and both are recorded per row. SurrealQL details: records carry `RecordID`s (a string id becomes a string key, BUGS F31); the two grouped graph OLAP queries use a subquery because core 2.3.10 (SDK 2.0.0) sorted by the group key after GROUP BY; Q1/Q6 with `math::sum`/`math::mean`/`GROUP ALL`; new-order as one `BEGIN ... COMMIT`; dense through `DEFINE INDEX ... HNSW DIMENSION n DIST EUCLIDEAN TYPE F32 EFC 100 M 16` and the `<\|k,ef\|>` operator. qDO landed TPC served, graph in both modes, and dense served at 1M (2026-09-14); the embedded TPC arm re-runs as qDU after F37, and the embedded 1M dense cell exceeded its budget and is named on the table. Every served row frozen so far has a blank disk cell (F38: the image is distroless, no `du`; the runner now sizes the volume from a helper container), and qDW, after qDV, re-runs the served cells for the disk reading. |
| ArangoDB | `arangodb@sha256:563cb2c07af0aead37fd688b58f51d6eb534a3da6163621e130e67d7a55176c4` (`arangodb:3.12.11`, 2026-08-31; the image reports "enterprise", which since 3.12 is the one self-managed build). License: BSL 1.1 per the repository's LICENSE at tag v3.12.11 (Licensed Work "ArangoDB Self Managed", Change License Apache-2.0 four years after each release), so not open source; the page's license table says so | 3.12.11, RocksDB storage | documents (TPC-C new-order as one stream transaction, TPC-H Q1/Q6 in AQL), graph (person collection keyed by id, KNOWS as an edge collection, LDBC reads and the three analytics as AQL traversals), dense (its vector index, which is FAISS IVF, not HNSW, matched by effect rather than by knob: nLists = round(4*sqrt(n)), and nProbe is calibrated in the cell on the held-out queries 1000:1200 (never the timed 1,000) to the smallest value whose recall@10 reaches the frozen ArcadeDB embedded fp32 median at the scale, cross-model; the row records seven fields, `ivf_nlists`, `ivf_nprobe`, `ivf_recall_target`, `ivf_recall_target_source`, `ivf_calibration_recall`, `ivf_calibration_queries`, and `ivf_calibration_slice`, so the operating point is auditable (see the ArangoDB smoke section below); `degree_family` says `ivf_flat_no_degree` and F7 accepts that), cross-model (vector hit in AQL, one-hop traversal, document updates in one stream transaction that the crash trial aborts) | served only: python-arango 8.3.5 is an HTTP client and the engine has no in-process mode, so one row per table, like MongoDB | bulk import API in batches; the server runs `arangod --vector-index true`, the 3.12 opt-in for the vector index. Queued as qDV after qDU (2026-09-13, DECISIONS #78). |
...
| l3d search | micro (5k) | 0 | p50 2.8 ms, nLists 71, nProbe 9 (sqrt(n) and an eighth of it: this smoke predates the 4*sqrt(n) rule and the in-cell calibration below) | 0.953 | 9.3 |
