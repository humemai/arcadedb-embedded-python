...
|---|---|---|---|---|---|
| ArcadeDB | wheel + `arcadedb-c25:<commit>` built from one commit | `26.9.1-dev · 8d6af9475` is September's pin; October re-pins to the matched pair built from the 26.10.1 release when it ships (DECISIONS #74), and the laptop skeleton runs the stock published image, which its rows name | all | embedded and served | Python package / Java API; sqlscript over HTTP when served |
