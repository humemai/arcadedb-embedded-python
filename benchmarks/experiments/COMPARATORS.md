...
| Qdrant | `qdrant/qdrant@sha256:0699e773…` | v1.19.1 | dense, sparse, composed cross-model | served (the composed stack runs the same binary beside Neo4j in one container; its row is below) | upsert batches over REST on 6333 (`qdrant-client`'s default transport; no arm asks for gRPC) |
...
| SurrealDB (served) | `surrealdb/surrealdb@sha256:6a500236…` (`v3.2.4`) | 3.2.4, RocksDB storage | documents (TPC), graph, dense, cross-model, time series (plain table) | served | SDK `insert()` and bulk relation insert over ws. Time series (2026-09-15): the same table, index and SurrealQL as the embedded arm, over ws through the reconnecting client (DECISIONS #91). Dense and cross-model: the HNSW index is defined before the load, which the 3.2.4 server fills in the background, so the build waits inside its index timer until a KNN probe's latency stops falling (`surreal_common.await_hnsw_settled`, recorded as `settle_s`; BUGS F134, DECISIONS #117). |
...
| Qdrant + Neo4j (the composed cross-model stack) | built image `dbbench:composed` (Dockerfile.composed) FROM `neo4j@sha256:e702d6b5…` with the Qdrant binary copied from `qdrant/qdrant@sha256:0699e773…`; both digests pinned in the Dockerfile (DECISIONS #117, BUGS F133) | Qdrant v1.19.1 + Neo4j 2026.08.1, stamped on the row as `qdrant:<version>+neo4j:<version>`, each read from its own server at connect | cross-model | served, one container holding both processes, so the one server cgroup caps, pins and measures both halves | Qdrant upsert batches over REST on 6333, then a payload index on `pid` and a wait for green status inside the index timer; Neo4j UNWIND batches over bolt. No transaction spans the two systems, which is what the arm is on the table to show. Qdrant runs with `RUN_MODE=production`, as its own image does, and at its HNSW defaults (m=16, ef_construct=100). |
...
The composed cross-model stack (Qdrant + Neo4j) is not an engine of its own: it is two of the rows above, at the same digests, wired together in one container (its row above). Its vector half was the Qdrant client's pure-Python in-memory local mode until 2026-09-26, so its rows up to then name `qdrant-local`: the September page's read `qdrant-local:1.19.0+neo4j:2026.07.1` (qDT re-ran it on 2026-09-16, 1.19.0 being the client package), and October's rows before `qOM` read `qdrant-local:1.19.1+neo4j:2026.08.1` and are marked `re-run` on the page until `qOM`'s rows against the Qdrant server supersede them.
...
| vertices | AGE 1.7.0 (PG17) | AGE 1.8.0 (PG18) | |
|---|---|---|---|
| 10,000 | 0.96 s | ~0.20 s | 4.8x |
| 50,000 | 1.18 s | ~0.19 s | 6.2x |
| 160,000 | 1.56 s | 0.18 s | 8.7x |
...
- **ArcadeDB is not a comparator** and is untouched here: it is pinned to our own wheel and the
  matching `arcadedb-c25:417314c18` server image at commit 417314c18d (DECISIONS #74), which
  the stage scripts `make_october_stages.py` writes export as `ARCADEDB_SERVER_IMAGE`.
...
|---|---|---|
| Neo4j 5-community (5.26.28) | `neo4j@sha256:4bae36af…` | 2026.07.1 on every arm, the composed cross-model row last (qDT, 2026-09-16), and then 2026.08.1 (`neo4j@sha256:e702d6b5…`) on every arm in the October re-pin; the October payload records 2026.08.1 for the composed row. |
