# Development log, June to August 2026

Historical notes from building and validating the harness, moved out of the README on
2026-09-07. Numbers here are laptop validation runs on the tiny tier, not paper numbers;
"tabular" is the name the paper later replaced with "documents"; the "planned layout" and
"remaining work" sections describe the state before the harness was complete. The paper's
numbers come from `results/runs_paper.csv` (ArcadeDB 26.8.1), see the README.
...
Standard caps: **cpuset 0-7 (8 P-cores), mem 8g (16g medium)** — same on the laptop and the benchmark host.
...
transactional CREATE. tiny (validated on the pre-fork 0.11.3 release; lane unchanged under
...
`prepared/` parquet; vector already has all three), (2) move to the benchmark host: clone repo, build images,
...
## Running (target: the benchmark host)

- Sync this repo to the benchmark host, run the orchestrator in the background (tmux/nohup) over SSH.
