...
- Small: cs.stackexchange.com (~650MB, 1.4M records)
- Medium: stats.stackexchange.com (~2.5GB, 5M records)
- Large: stackoverflow.com (~325GB, 350M records)

Usage:
    python 07_stackoverflow_multimodel.py --dataset small --phase 1
    python 07_stackoverflow_multimodel.py --dataset small --phase 2
    python 07_stackoverflow_multimodel.py --dataset small --phase 3
"""
...
class StackOverflowDatabase:
    ...
    DATASET_PATHS = {
        'small': Path.home() / 'data' / 'stackexchange' / 'cs.stackexchange.com',
        'medium': Path.home() / 'data' / 'stackexchange' / 'stats.stackexchange.com',
        'large': Path.home() / 'data' / 'stackexchange' / 'stackoverflow.com-Posts',
    }
    ...
    def create_document_types(self):
        ...
        for doc_type, properties in self.DOCUMENT_SCHEMAS.items():
            ...
            for prop_name, prop_type in properties.items():
                self.db.command(
                    f"CREATE PROPERTY {doc_type}.{prop_name} {prop_type}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Phase 1: Import documents from small dataset
  python 07_stackoverflow_multimodel.py --dataset small --phase 1
  
  # Phase 2: Build graph from documents
  python 07_stackoverflow_multimodel.py --dataset small --phase 2
  
  # Phase 3: Add vector embeddings
  python 07_stackoverflow_multimodel.py --dataset small --phase 3
  
  # Run all phases
  python 07_stackoverflow_multimodel.py --dataset small --phase 1 2 3
        """
    )
    ...
    parser.add_argument(
        ...
        choices=[1, 2, 3],
        default=[1],
        help='Phases to execute (default: 1)'
    )
    ...
    parser.add_argument(
        '--schema',
        help='Path to schema JSON file (optional)'
    )
