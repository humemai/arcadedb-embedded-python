...
for run_dir in sorted(glob.glob(os.path.join(input_dir, "*"))):
    ...
    status_label = status_obj.get("search_run_label") or status_obj.get("run_label")
    if status_label:
        status_by_run[str(status_label)] = status_obj
        run_dir_by_run[str(status_label)] = run_dir
...
for result_path in sorted(glob.glob(os.path.join(input_dir, "*", "search_results_*.json"))):
    ...
    status_obj = status_by_run.get(str(run_label) if run_label else "")
        ...
        rows.append(
            {
                ...
                "backend": canonicalize_backend_label(db_obj.get("backend")),
                "run_label": run_label,
                "seed": to_int(search_obj.get("seed")),
                "mem_limit": search_obj.get("mem_limit") or run_obj.get("mem_limit") or telemetry_obj.get("mem_limit") or ((status_obj or {}).get("mem_limit") if isinstance(status_obj, dict) else None) or parse_mem_limit_from_dir_name(run_dir) or parse_mem_limit_from_run_label(run_label),
                ...
                "du_mib": du_mib,
                "status": (status_obj or {}).get("status") or telemetry_obj.get("run_status") or "success",
            }
        )
    ...
    rows.append(
        {
            ...
            "k": to_int(status.get("k")),
            "overquery_factor": None,
            "effective_ef_search": None,
            "effective_nprobes": None,
            "queries": None,
            "recall_mean": None,
...
for run_dir in run_dirs_no_status:
    if glob.glob(os.path.join(run_dir, "search_results_*.json")):
        continue
...
if not rows:
    print("No matching search_run_status.json or search_results_*.json files found.", file=sys.stderr)
    sys.exit(2)
