...
BACKENDS_RAW="faiss,lancedb,pgvector,qdrant,milvus,bruteforce,arcadedb_sql"
...
for ((run = 1; run <= RUNS; run++)); do
    for backend in "${BACKENDS[@]}"; do
        if [[ "$backend" == "bruteforce" ]]; then
            ...
            build_run_label=$(printf "%s_r%02d_%s_s%05d_m%s" "$BUILD_LABEL_PREFIX" "$run" "$backend" "$seed" "$mem_tag")
            db_path="$DB_ROOT/backend=${backend}_dataset=${DATASET}_mem=${MEM_LIMIT}_run=${build_run_label}"
            mkdir -p "$db_path"
        else
            ...
            if [[ "$build_run_label" =~ _s([0-9]{5})(_m[[:alnum:]]+)?$ ]]; then
                seed=$((10#${BASH_REMATCH[1]}))
            else
                seed=$((SEED_START + execution_idx))
            fi
        fi
        search_run_label=$(printf "%s_r%02d_%s_s%05d_m%s" "$SEARCH_LABEL_PREFIX" "$run" "$backend" "$seed" "$mem_tag")
