...
def resolve_db_label(data, status_for_run):
    status_for_run = status_for_run if isinstance(status_for_run, dict) else {}
    status_db = canonicalize_db_label(status_for_run.get("db"))
    status_engine = canonicalize_db_label(status_for_run.get("db_engine"))
    status_lang = str(status_for_run.get("arcadedb_oltp_language") or "").strip().lower()
    run_label_s = str((data or {}).get("run_label") or status_for_run.get("run_label") or "")

    data_db = canonicalize_db_label((data or {}).get("db"))
    data_lang = str((data or {}).get("arcadedb_oltp_language") or "").strip().lower()
    lang = data_lang or status_lang

    if status_db in ("arcadedb_sql", "arcadedb_cypher"):
        return status_db
    ...
    arcadedb_like = (status_engine == "arcadedb") or (status_db == "arcadedb") or (data_db == "arcadedb")
    if arcadedb_like and lang in ("sql", "cypher"):
...
def parse_run_dir_metadata(run_dir):
    base = os.path.basename(str(run_dir or "").rstrip("/"))
    if not base or "_graph_oltp_" not in base:
        return {}

    dataset_part, remainder = base.split("_graph_oltp_", 1)
...
for status in status_rows:
    ...
    row = {
        ...
        "load_node_count": None,
        "load_edge_count": None,
        "schema_time_s": None,
        "index_time_s": None,
        "load_time_s": None,
        "counts_time_s": None,
...
status_no_result_rows = sum(1 for row in rows if row.get("status") == "no_result")
