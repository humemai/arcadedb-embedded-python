...
def parse_run_dir_metadata(run_dir):
    base = os.path.basename(str(run_dir or "").rstrip("/"))
    if not base or "_tables_olap_" not in base:
        return {}

    dataset_part, remainder = base.split("_tables_olap_", 1)
...
for run_dir in run_dirs:
    ...
    for result_path in result_paths:
        ...
        row = {
            ...
            "db": canonicalize_db_label(data.get("db")),
            "run_label": data.get("run_label"),
            ...
            or (status_by_run.get(str(data.get("run_label")) or "") or {}).get("mem_limit")
            or parse_mem_limit_from_dir_name(run_dir)
            or parse_mem_limit_from_run_label(data.get("run_label")),
            ...
            "du_mib": du_mib,
            "status": (status_by_run.get(str(data.get("run_label")) or "") or {}).get("status") or "success",
            "exit_code": to_int((status_by_run.get(str(data.get("run_label")) or "") or {}).get("exit_code")),
    ...
    result_rows.append(
        {
            ...
            "run_label": rl,
            "seed": meta.get("seed"),
            "threads": meta.get("threads") or parse_threads_from_run_label(rl),
            "batch_size": None,
            "query_runs": None,
            "query_order": None,
...
for current_dataset in datasets:
    ...
    lines.append("")
    lines.append(
        f"| db | run_label | seed | runs | threads | mem_limit | query_runs | query_order | status | {load_col} | {index_col} | {query_col} | {rss_col} | {du_col} |"
    )
    lines.append("|---|---|---|---:|---:|---|---:|---|---|---:|---:|---:|---:|---:|")
