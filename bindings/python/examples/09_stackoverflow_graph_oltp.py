...
def arcadedb_insert_vertices(db, vertex_type: str, rows: List[Dict[str, Any]]):
    ...
    for offset in range(0, len(rows), chunk_size):
        ...
        set_clause = ", ".join(f"{key} = :{key}" for key in keys)
        statement = f"INSERT INTO {vertex_type} SET {set_clause}"
        with db.transaction():
            for row in chunk:
                db.command("sql", statement, row)
...
def verify_single_thread_series(
    ...
    arcadedb_oltp_language = (
        args.arcadedb_oltp_language
        if db_label in ("arcadedb", "arcadedb_sql", "arcadedb_cypher")
        else None
    )
    ...
    for offset in range(0, len(rows), chunk_size):
        ...
        with db.transaction():
            for row in chunk:
                ...
                if from_rid is not None and to_rid is not None:
                    statement = (
                    ...
                    statement = (
                        ...
                        f"FROM (SELECT FROM {from_label} WHERE Id = :from_id) "
                        f"TO (SELECT FROM {to_label} WHERE Id = :to_id)"
                    )
...
def main():
    ...
    if args.db == "arcadedb_sql":
        args.arcadedb_oltp_language = "sql"
    elif args.db == "arcadedb_cypher":
        args.arcadedb_oltp_language = "cypher"
    else:
        args.arcadedb_oltp_language = "cypher"
    ...
    mem_tag = re.sub(r"[^A-Za-z0-9]+", "", args.mem_limit or "") or "default"
    db_name = f"{args.dataset.replace('-', '_')}_graph_oltp_{args.db}_mem{mem_tag}"
    if args.run_label:
