...
- It sweeps `overquery_factor` values.
...
### Overquery Normalization

For HNSW-style engines, the benchmark maps `overquery_factor` to an effective
`ef_search` using:

```python
max(int(k), int(round(HNSW_EF_NORMALIZATION * k * overquery_factor)))
```
...
```python
index.find_nearest_approximate(qvec, k=k, overquery_factor=effective_overquery)
```

- non-product paths use:

```python
index.find_nearest(qvec, k=k, overquery_factor=effective_overquery)
```
...
- Example 12 normalizes the search knobs, but `overquery_factor` does not mean exactly the same thing in every backend.
