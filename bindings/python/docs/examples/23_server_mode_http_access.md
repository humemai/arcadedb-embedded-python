# 23 - Server Mode And HTTP Access

[View source code]({{ config.repo_url }}/blob/{{ config.extra.version_tag }}/bindings/python/examples/23_server_mode_http_access.py){ .md-button }
...
```bash
python3 23_server_mode_http_access.py
```

Keep the server open so you can inspect Studio manually:

```bash
python3 23_server_mode_http_access.py --wait-for-enter
```

With a custom port and server root:

```bash
python3 23_server_mode_http_access.py --http-port 2491 --server-root ./my_test_databases/server_demo_alt
```
